...
# Enable repository-local runtime patches (no site-packages edits).
export VERL_SGLANG_WASTE_SD_PATCH=1
export VERL_SGLANG_STANDALONE_SYNC_MODE="${VERL_SGLANG_STANDALONE_SYNC_MODE:-draft_only}"
# NOTE: strict mode is driven by Hydra config `distill.strict` in main_waste_sd.py.
...
WASTE_SD_VAL_DO_SAMPLE="${WASTE_SD_VAL_DO_SAMPLE:-}"
WASTE_SD_VAL_TOP_P="${WASTE_SD_VAL_TOP_P:-}"
WASTE_SD_VAL_TOP_K="${WASTE_SD_VAL_TOP_K:-}"
...
# Tokenizer init on rollout server:
# - default: True (preserve existing training behavior)
# - eval-only teacher-target path often benefits from False for closer parity with native serve.
WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT="${WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT:-}"
...
WASTE_SD_DISTILL_STRICT="${WASTE_SD_DISTILL_STRICT:-true}"
WASTE_SD_ACTOR_LR="${WASTE_SD_ACTOR_LR:-1e-6}"
WASTE_SD_AGENT_LOOP_CONFIG_PATH="${WASTE_SD_AGENT_LOOP_CONFIG_PATH:-${REPO_ROOT}/recipe/waste_sd/config/agent_loop_waste_sd.yaml}"
...
# Eval-only toggle: force rollout target model to teacher/ref in speculative decoding.
# This keeps training behavior unchanged unless explicitly enabled for block-eval-only runs.
WASTE_SD_BLOCK_EVAL_USE_TEACHER_TARGET="${WASTE_SD_BLOCK_EVAL_USE_TEACHER_TARGET:-0}"
WASTE_SD_ENABLE_THINKING="${WASTE_SD_ENABLE_THINKING:-}"
...
if is_true "${WASTE_SD_BLOCK_EVAL_ONLY}" && is_true "${WASTE_SD_BLOCK_EVAL_USE_TEACHER_TARGET}"; then
  TRAINER_EXTRA_OVERRIDES+=("distill.rollout_target=local_ref")
  TRAINER_EXTRA_OVERRIDES+=("+distill.off_policy_require_ref=true")
  TRAINER_EXTRA_OVERRIDES+=("actor_rollout_ref.model.tokenizer_path=${WASTE_SD_TEACHER_MODEL}")
fi

if [[ -z "${WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT}" ]]; then
  if is_true "${WASTE_SD_BLOCK_EVAL_ONLY}" && is_true "${WASTE_SD_BLOCK_EVAL_USE_TEACHER_TARGET}"; then
    WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT="false"
  else
    WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT="true"
  fi
fi
...
if [[ -n "${WASTE_SD_VAL_DO_SAMPLE}" ]]; then
  VAL_SAMPLING_OVERRIDES+=("actor_rollout_ref.rollout.val_kwargs.do_sample=${WASTE_SD_VAL_DO_SAMPLE}")
fi
if [[ -n "${WASTE_SD_VAL_TOP_P}" ]]; then
  VAL_SAMPLING_OVERRIDES+=("actor_rollout_ref.rollout.val_kwargs.top_p=${WASTE_SD_VAL_TOP_P}")
fi
if [[ -n "${WASTE_SD_VAL_TOP_K}" ]]; then
  VAL_SAMPLING_OVERRIDES+=("actor_rollout_ref.rollout.val_kwargs.top_k=${WASTE_SD_VAL_TOP_K}")
fi

DATA_TEMPLATE_OVERRIDES=()
if [[ -n "${WASTE_SD_ENABLE_THINKING}" ]]; then
  DATA_TEMPLATE_OVERRIDES+=("++data.apply_chat_template_kwargs.enable_thinking=${WASTE_SD_ENABLE_THINKING}")
fi
...
if [[ -n "${VERL_SGLANG_SD_DEBUG_SIG:-}" ]]; then
  RUNTIME_ENV_OVERRIDES+=("+ray_kwargs.ray_init.runtime_env.env_vars.VERL_SGLANG_SD_DEBUG_SIG=${VERL_SGLANG_SD_DEBUG_SIG}")
fi
...
"${PYTHON_BIN}" -m recipe.waste_sd.main_waste_sd \
  actor_rollout_ref.model.path="${WASTE_SD_STUDENT_MODEL}" \
  +actor_rollout_ref.ref.model.path="${WASTE_SD_TEACHER_MODEL}" \
  +actor_rollout_ref.rollout.engine_kwargs.sglang.speculative_algorithm=STANDALONE \
  +actor_rollout_ref.rollout.engine_kwargs.sglang.speculative_draft_model_path="${WASTE_SD_STUDENT_MODEL}" \
  +actor_rollout_ref.rollout.engine_kwargs.sglang.speculative_draft_model_revision=main \
  +actor_rollout_ref.rollout.engine_kwargs.sglang.speculative_draft_load_format=auto \
  +actor_rollout_ref.rollout.engine_kwargs.sglang.speculative_num_steps="${WASTE_SD_GAMMA}" \
  ...
  actor_rollout_ref.rollout.skip_tokenizer_init="${WASTE_SD_ROLLOUT_SKIP_TOKENIZER_INIT}" \
  ...
  distill.strict="${WASTE_SD_DISTILL_STRICT}" \
  ...
  actor_rollout_ref.rollout.agent.agent_loop_config_path="${WASTE_SD_AGENT_LOOP_CONFIG_PATH}" \

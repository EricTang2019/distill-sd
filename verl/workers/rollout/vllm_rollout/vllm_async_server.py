...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        self.config: RolloutConfig = omega_conf_to_dataclass(config, dataclass_type=RolloutConfig)
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        prometheus_cfg = self.config.prometheus
        if isinstance(prometheus_cfg, dict):
            prometheus_enable = bool(prometheus_cfg.get("enable", False))
            prometheus_served_model_name = prometheus_cfg.get("served_model_name")
        else:
            prometheus_enable = bool(getattr(prometheus_cfg, "enable", False))
            prometheus_served_model_name = getattr(prometheus_cfg, "served_model_name", None)

        if prometheus_enable:
            if prometheus_served_model_name:
                # Extract model name from path if it's a full path
                served_model_name = prometheus_served_model_name
    ...
    async def generate(
        ...
        collect_spec_trace = self._should_collect_spec_trace()
        sampling_params["logprobs"] = 0 if sampling_params.pop("logprobs", False) else None
        if collect_spec_trace:
            # In DELTA mode vLLM yields only the newly accepted chunk from each
            # decode step, which matches waste_sd block tracing semantics.
        ...
        delta_log_probs: list[float] = []
        delta_routed_experts: list[Any] = []
        async for output in generator:
            final_res = output
            if not collect_spec_trace:
                continue

            delta_token_ids = [int(token_id) for token_id in output.outputs[0].token_ids]
            if not delta_token_ids:
                continue

            delta_token_chunks.append(delta_token_ids)

            if sampling_params.logprobs is not None:
                chunk_log_probs = extract_token_log_probs(delta_token_ids, output.outputs[0].logprobs)
                if chunk_log_probs is not None:
                    delta_log_probs.extend(chunk_log_probs)

            if self.config.enable_rollout_routing_replay and output.outputs[0].routed_experts is not None:
                delta_routed_experts.append(output.outputs[0].routed_experts)
        assert final_res is not None

        spec_accept_lens = None
        spec_verify_ct = None
        spec_accepted_tokens = None
        if collect_spec_trace:
            traced = summarize_speculative_chunks(delta_token_chunks)
            token_ids = traced["token_ids"]
            spec_accept_lens = traced["spec_accept_lens"]
            spec_verify_ct = traced["spec_verify_ct"]
            spec_accepted_tokens = traced["spec_accepted_tokens"]
            log_probs = delta_log_probs if sampling_params.logprobs is not None else None
            if log_probs is not None and len(log_probs) != len(token_ids):
                logger.warning(
                    "vLLM DELTA logprobs length mismatch in speculative tracing: "
                    "len(log_probs)=%s vs len(token_ids)=%s for request_id=%s. Dropping logprobs.",
                    len(log_probs),
                    len(token_ids),
                    request_id,
                )
            ...
            log_probs = None
            if sampling_params.logprobs is not None:
                log_probs = extract_token_log_probs(token_ids, final_res.outputs[0].logprobs)

        routed_experts = None
        if self.config.enable_rollout_routing_replay:
            if collect_spec_trace:
                routed_experts = concat_optional_arrays(delta_routed_experts)
            else:
    ...
    def _should_collect_spec_trace(self) -> bool:
        mtp_enabled = bool(self.config.mtp.enable and self.config.mtp.enable_rollout)
        engine_kwargs = self.config.get("engine_kwargs", {}).get("vllm", {}) or {}
        return mtp_enabled or bool(engine_kwargs.get("speculative_config"))

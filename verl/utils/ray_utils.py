...
def ray_noset_visible_devices(env_vars=os.environ):
    ...
    # Keep semantics aligned with Ray's env_bool:
    # only "1"/"true" (case-insensitive) means enabled.
    def _env_bool_like_ray(value):
        if value is None:
            return False
        value_str = str(value)
        return value_str == "1" or value_str.lower() == "true"

    return any(_env_bool_like_ray(env_vars.get(env_var)) for env_var in NOSET_VISIBLE_DEVICES_ENV_VARS_LIST)

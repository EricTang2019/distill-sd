...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        if self.config.layered_summon or (
            self.config.expert_parallel_size > 1 and not _check_vllm_version_for_sleep_level()
        ):
        ...
        # Use the same namespace rule as vLLM worker extension side.
        self.zmq_namespace = get_zmq_namespace()
        self.zmq_handle = get_zmq_handle(self.device_uuid)
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        checkpoint_engine_cfg = self.config.get("checkpoint_engine", None)
        if isinstance(checkpoint_engine_cfg, dict):
            bucket_size_mb = checkpoint_engine_cfg.get("update_weights_bucket_megabytes")
        else:
            bucket_size_mb = getattr(checkpoint_engine_cfg, "update_weights_bucket_megabytes", None)
        if bucket_size_mb is None:
            # Backward compatibility with legacy rollout config field.
            bucket_size_mb = self.config.get("update_weights_bucket_megabytes", None)
        if bucket_size_mb is None:
            raise ValueError("Missing rollout checkpoint bucket size: update_weights_bucket_megabytes")
        bucket_size_mb = int(bucket_size_mb)
        bucket_size = int(bucket_size_mb) << 20
        timeout_ms = get_zmq_timeout_ms()
        ...
        # IPC socket file may remain after abnormal exits; retry once after cleanup.
        ...
        try:
            ...
            s.bind(self.zmq_handle)
        logger.warning(
            "vLLM update_weights sender bound %s (timeout=%ss, use_shm=%s)",
            self.zmq_handle,
            timeout_sec,
            self.use_shm,
        )

        def _send(obj: object, stage: str) -> None:
            try:
                s.send_pyobj(obj)
            except zmq.Again as e:
                raise TimeoutError(
                    f"Timed out sending {stage} on {self.zmq_handle} after {timeout_sec}s."
                ) from e

        def _recv(stage: str) -> None:
            try:
                s.recv()
            except zmq.Again as e:
                raise TimeoutError(
                    f"Timed out waiting for {stage} ack on {self.zmq_handle} after {timeout_sec}s."
                ) from e
        ...
        if not self.use_shm:
            ...
            _send(comm_metadata, "shared memory metadata")
        ...
        async for name, weight in ensure_async_iterator(weights):
            ...
            if offset + weight.nbytes > bucket_size:
                ...
                _send({"bucket_meta": bucket_meta, "is_last": False}, "weight bucket")
                _recv("weight bucket")
                bucket_meta = {}
                offset = 0
        ...
        get_torch_device().synchronize()
        _send({"bucket_meta": bucket_meta, "is_last": True}, "last weight bucket")
        _recv("last weight bucket")

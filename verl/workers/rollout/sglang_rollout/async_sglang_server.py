...
from sglang.srt.entrypoints.engine import (
    run_detokenizer_process as _orig_run_detokenizer_process,
)
from sglang.srt.entrypoints.engine import (
    run_scheduler_process as _orig_run_scheduler_process,
)
...
from verl.workers.rollout.sglang_rollout.standalone_patch import apply_standalone_target_only_patch
...
def _run_scheduler_process_with_verl_patches(*args, **kwargs):
    # Ensure speculative + metadata patches are applied in the spawned scheduler process.
    logger.warning(
        "VERL scheduler wrapper active in pid=%s: applying Waste-SD and standalone patches before scheduler start.",
        os.getpid(),
    )
    apply_waste_sd_patch()
    apply_standalone_target_only_patch()
    return _orig_run_scheduler_process(*args, **kwargs)


def _run_detokenizer_process_with_verl_patches(*args, **kwargs):
    # Detokenizer process also needs Waste-SD metadata propagation hooks.
    logger.warning(
        "VERL detokenizer wrapper active in pid=%s: applying Waste-SD patch before detokenizer start.",
        os.getpid(),
    )
    apply_waste_sd_patch()
    return _orig_run_detokenizer_process(*args, **kwargs)
...
class SGLangHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # Ensure spawned scheduler/detokenizer processes execute with VERL patches enabled.
        sglang.srt.entrypoints.engine.run_scheduler_process = _run_scheduler_process_with_verl_patches
        sglang.srt.entrypoints.engine.run_detokenizer_process = _run_detokenizer_process_with_verl_patches
        ...
        if version.parse(sglang.__version__) >= version.parse("0.5.7"):
            self.tokenizer_manager, self.template_manager, self.scheduler_info, *_ = _launch_subprocesses(
                ...
                run_scheduler_process_func=_run_scheduler_process_with_verl_patches,
                run_detokenizer_process_func=_run_detokenizer_process_with_verl_patches,
            )
        else:
            self.tokenizer_manager, self.template_manager, self.scheduler_info, *_ = _launch_subprocesses(
                server_args=server_args
    ...
    async def generate(
        ...
        output = None
        async for frame in self.tokenizer_manager.generate_request(generate_request, None):
            output = frame
        if output is None:
            raise RuntimeError("SGLang generate_request returned no frames")
        if return_logprob:
        ...
        return TokenOutput(
            ...
            spec_accepted_tokens=meta_info.get("spec_accepted_tokens"),
            target_forward_total_calls=meta_info.get("target_forward_total_calls"),
            target_forward_verify_true_calls=meta_info.get("target_forward_verify_true_calls"),
            target_forward_verify_false_calls=meta_info.get("target_forward_verify_false_calls"),
            target_forward_verify_true_called=meta_info.get("target_forward_verify_true_called"),
            verify_hook_calls=meta_info.get("verify_hook_calls"),
        )

...
export WASTE_SD_RAY_TMP_DIR="${WASTE_SD_RAY_TMP_DIR:-${TMPDIR:-/tmp}/rwsd_${USER}_$$}"
...
if (( ${#WASTE_SD_RAY_TMP_DIR} + _RAY_SOCKET_SUFFIX_LEN > 107 )); then
  _fallback_tmp_dir="/tmp/rwsd_${USER}_$$"
...
# Off-policy teacher rollout baseline does not require Waste-SD runtime patches.
...
WASTE_SD_TEACHER_FORWARD_BACKEND="${WASTE_SD_TEACHER_FORWARD_BACKEND:-fsdp_ref}"
WASTE_SD_DISTILL_LOSS_TYPE="${WASTE_SD_DISTILL_LOSS_TYPE:-fkl}"
WASTE_SD_WEIGHTING_MODE="${WASTE_SD_WEIGHTING_MODE:-uniform_mean}"
WASTE_SD_KL_FLOOR_COEF="${WASTE_SD_KL_FLOOR_COEF:-0.0}"
WASTE_SD_REMBUDGET_TVD_OBJECTIVE_MODE="${WASTE_SD_REMBUDGET_TVD_OBJECTIVE_MODE:-weighted_mean}"
WASTE_SD_REMBUDGET_TVD_BACKWARD_LENGTH_NORMALIZE="${WASTE_SD_REMBUDGET_TVD_BACKWARD_LENGTH_NORMALIZE:-false}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF:-0.0}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_SCHEDULE="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_SCHEDULE:-constant}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START:-${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF}}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END:-${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF}}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START_STEP="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START_STEP:-0}"
WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END_STEP="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END_STEP:-${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START_STEP}}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF:-0.0}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_SCHEDULE="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_SCHEDULE:-constant}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START:-${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF}}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END:-${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF}}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START_STEP="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START_STEP:-0}"
WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END_STEP="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END_STEP:-${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START_STEP}}"
WASTE_SD_LOG_CURRENT_BATCH_THEOREM_RB_TVD="${WASTE_SD_LOG_CURRENT_BATCH_THEOREM_RB_TVD:-false}"
WASTE_SD_LOG_POST_UPDATE_BATCH_THEOREM_RB_TVD="${WASTE_SD_LOG_POST_UPDATE_BATCH_THEOREM_RB_TVD:-false}"
...
WASTE_SD_EXACT_BLOCKS_EVAL_FREQ="${WASTE_SD_EXACT_BLOCKS_EVAL_FREQ:--1}"
WASTE_SD_EXACT_BLOCKS_EVAL_DATA_FILE="${WASTE_SD_EXACT_BLOCKS_EVAL_DATA_FILE:-}"
WASTE_SD_EXACT_BLOCKS_EVAL_BATCH_SIZE="${WASTE_SD_EXACT_BLOCKS_EVAL_BATCH_SIZE:-64}"
WASTE_SD_EXACT_BLOCKS_EVAL_MAX_SAMPLES="${WASTE_SD_EXACT_BLOCKS_EVAL_MAX_SAMPLES:--1}"
WASTE_SD_EXACT_BLOCKS_EVAL_MAX_PROMPT_LENGTH="${WASTE_SD_EXACT_BLOCKS_EVAL_MAX_PROMPT_LENGTH:-1024}"
WASTE_SD_EXACT_BLOCKS_EVAL_MAX_RESPONSE_LENGTH="${WASTE_SD_EXACT_BLOCKS_EVAL_MAX_RESPONSE_LENGTH:-2048}"
WASTE_SD_EXACT_BLOCKS_EVAL_TRUNCATION="${WASTE_SD_EXACT_BLOCKS_EVAL_TRUNCATION:-right}"
WASTE_SD_EXACT_BLOCKS_EVAL_TEMPERATURE="${WASTE_SD_EXACT_BLOCKS_EVAL_TEMPERATURE:-1.0}"
...
"${PYTHON_BIN}" -m recipe.waste_sd.main_teacher_off_policy \
  ...
  distill.loss_type="${WASTE_SD_DISTILL_LOSS_TYPE}" \
  distill.weighting_mode="${WASTE_SD_WEIGHTING_MODE}" \
  distill.kl_floor_coef="${WASTE_SD_KL_FLOOR_COEF}" \
  distill.rembudget_tvd_objective_mode="${WASTE_SD_REMBUDGET_TVD_OBJECTIVE_MODE}" \
  distill.rembudget_tvd_backward_length_normalize="${WASTE_SD_REMBUDGET_TVD_BACKWARD_LENGTH_NORMALIZE}" \
  distill.rembudget_unweighted_kl_coef="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF}" \
  distill.rembudget_unweighted_kl_coef_schedule="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_SCHEDULE}" \
  distill.rembudget_unweighted_kl_coef_start="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START}" \
  distill.rembudget_unweighted_kl_coef_end="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END}" \
  distill.rembudget_unweighted_kl_coef_start_step="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_START_STEP}" \
  distill.rembudget_unweighted_kl_coef_end_step="${WASTE_SD_REMBUDGET_UNWEIGHTED_KL_COEF_END_STEP}" \
  distill.rembudget_tvd_unweighted_fkl_coef="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF}" \
  distill.rembudget_tvd_unweighted_fkl_coef_schedule="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_SCHEDULE}" \
  distill.rembudget_tvd_unweighted_fkl_coef_start="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START}" \
  distill.rembudget_tvd_unweighted_fkl_coef_end="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END}" \
  distill.rembudget_tvd_unweighted_fkl_coef_start_step="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_START_STEP}" \
  distill.rembudget_tvd_unweighted_fkl_coef_end_step="${WASTE_SD_REMBUDGET_TVD_UNWEIGHTED_FKL_COEF_END_STEP}" \
  distill.log_current_batch_theorem_rb_tvd="${WASTE_SD_LOG_CURRENT_BATCH_THEOREM_RB_TVD}" \
  distill.log_post_update_batch_theorem_rb_tvd="${WASTE_SD_LOG_POST_UPDATE_BATCH_THEOREM_RB_TVD}" \
  distill.q_source=local_ref \
  distill.strict=true \
  distill.gamma="${WASTE_SD_GAMMA}" \
  distill.teacher_forward_backend="${WASTE_SD_TEACHER_FORWARD_BACKEND}" \

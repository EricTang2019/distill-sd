...
class VLLMEngine:
    ...
    def get_topk_logprobs(self, prompt_token_ids, temperature=0.8, max_new_tokens=1, only_response=False):
        ...
        outputs = self._generate(prompt_token_ids=prompt_token_ids, sampling_params=sampling_params)
    ...
    def _generate(self, prompt_token_ids, sampling_params):
        """Call vLLM generate with backward/forward-compatible argument names."""
        prompt_inputs = self._build_prompt_inputs(prompt_token_ids)
        candidate_kwargs = (
        ...
        for kwargs in candidate_kwargs:
            try:
                return self.llm.generate(sampling_params=sampling_params, **kwargs)
        ...
        raise RuntimeError(
            "vLLM generate signature is incompatible with GKD teacher engine "
            "(tried prompt_token_ids/prompts/inputs)."
        ) from last_type_error

    @staticmethod
    def _build_prompt_inputs(prompt_token_ids):
        """Build vLLM-compatible prompt objects from token-id batches."""
        ...
        # Normalize to batch shape: list[list[int]]
        if isinstance(prompt_token_ids[0], int):
            token_batches = [prompt_token_ids]
        else:
            token_batches = prompt_token_ids
        ...
        try:
            ...
            return [TokensPrompt(prompt_token_ids=tokens) for tokens in token_batches]
        except Exception:
            # Fallback for versions where a plain mapping is accepted.
            return [{"prompt_token_ids": tokens} for tokens in token_batches]

...
class WasteSDTeacherOnPolicyTaskRunner(WasteSDTaskRunner):
    """Task runner for strict on-policy teacher distillation baseline."""


@hydra.main(config_path="config", config_name="waste_sd_trainer", version_base=None)
def main(config):
    # This baseline does not use Waste-SD speculative runtime patch.
    ...
    if not isinstance(ref_model_path, str) or not ref_model_path.strip():
        raise ValueError(
            "Teacher on-policy baseline requires actor_rollout_ref.ref.model.path to be explicitly set. "
            "Refusing actor-path fallback for teacher logits."
        )
    ...
    with open_dict(config):
        ...
        config.distill.rollout_target = "actor"
        # Rollout mode is async-only in current verl. Enforce strictness via staleness filtering.
        config.distill.staleness_max_version_gap = 0

        # Disable speculative/MTP rollout even if inherited from other launchers.
    ...
    run_waste_sd(
        config,
        task_runner_cls=WasteSDTeacherOnPolicyTaskRunner,

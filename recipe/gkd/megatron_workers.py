...
try:
    ...
    class Profiler:
        """Compatibility no-op profiler for newer verl profiler API."""
...
class OnPolicyDistillActor:
    ...
    def forward_backward_batch(
        ...
        # Newer tensordict/dataflow paths can pass a locked TensorDict.
        # Clone to get a writable view for in-place key updates below.
...
class MegatronOnPolicyDistillActorWorker(ActorRolloutRefWorker):
    ...
    def sync_rollout_weights(self):
        ...
        bucket_mb = OmegaConf.select(self.config, "rollout.checkpoint_engine.update_weights_bucket_megabytes")
        if bucket_mb is None:
            bucket_mb = self.config.rollout.update_weights_bucket_megabytes
...
class MegatronOnPolicyDistillRolloutWorker(ActorRolloutRefWorker):
    ...
    def sync_rollout_weights(self):
        ...
        # New rollout API (ServerAdapter): use rollout.update_weights(...) instead of
        # touching rollout engine internals (e.g. inference_engine/model_runner).
        if hasattr(self.rollout, "update_weights"):
            ...
            async def _update_weights_via_adapter():
                await self.rollout.update_weights(per_tensor_generator(), peft_config=None, base_sync_done=True)
            ...
            loop.run_until_complete(_update_weights_via_adapter())
            return

        # Legacy fallback path for old rollout implementations.

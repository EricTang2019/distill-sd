...
class Worker(WorkerHelper):
    ...
    def _setup_env_cuda_visible_devices(self):
        ...
        debug_device_binding = os.environ.get("VERL_DEBUG_DEVICE_BINDING", "0") == "1"
        ...
        def _infer_local_rank(default=None):
            local_rank = _parse_int_env("LOCAL_RANK")
            if local_rank is not None:
                return local_rank

            rank = _parse_int_env("RANK")
            local_world_size = _parse_int_env("LOCAL_WORLD_SIZE")
            if local_world_size is None:
                local_world_size = _parse_int_env("RAY_LOCAL_WORLD_SIZE")

            if rank is not None and local_world_size and local_world_size > 0:
        ...
        if is_ray_noset_visible_devices:
            ...
            local_rank = _infer_local_rank(default=None)

            if local_rank is None:
                device_name = "NPU" if is_npu_available else "GPU"
                accelerator_ids = ray.get_runtime_context().get_accelerator_ids().get(device_name, [])
                raw_rank = accelerator_ids[0] if accelerator_ids else 0
                try:
                    physical_rank = int(raw_rank)
                except (TypeError, ValueError):
                    try:
                        physical_rank = int(float(raw_rank))
                    except (TypeError, ValueError):
                        physical_rank = 0

                # Under CUDA_VISIBLE_DEVICES=a,b,..., torch local ordinals are 0..N-1.
                # Ray accelerator IDs may report physical ids (e.g. 4/5), so map them.
                if cuda_val and not is_npu_available:
                    visible_devices = [dev.strip() for dev in cuda_val.split(",") if dev.strip()]
                    local_rank = None
                    for idx, dev in enumerate(visible_devices):
                        try:
                            if int(dev) == physical_rank:
                                local_rank = idx
                                break
                        except (TypeError, ValueError):
                            continue
                    if local_rank is None:
                        if 0 <= physical_rank < len(visible_devices):
                            local_rank = physical_rank
                        else:
                            local_rank = 0
                else:
                    local_rank = physical_rank
        ...
        # In some Ray/runtime combinations, CUDA_VISIBLE_DEVICES may contain
        # multiple devices in a worker process. Bind deterministically by
        # LOCAL_RANK to avoid multiple ranks selecting the same logical device 0.
        if cuda_val:
            visible_devices = [dev.strip() for dev in cuda_val.split(",") if dev.strip()]
            if len(visible_devices) == 1:
                get_torch_device().set_device(0)
            elif len(visible_devices) > 1:
                local_rank = _infer_local_rank(default=0)
                get_torch_device().set_device(int(local_rank) % len(visible_devices))

        if debug_device_binding:
            try:
                torch_device = get_torch_device()
                current_device = int(torch_device.current_device())
                device_info = {"current_device": current_device}
                ...
                print(
                    ...
                    f"ray_noset_visible_devices={is_ray_noset_visible_devices} "
                    f"cuda_visible_devices={os.environ.get('CUDA_VISIBLE_DEVICES', '')} "
                    f"device={device_info}",
                    flush=True,
                )
            except Exception as e:
                print(f"[VERL_DEVICE_BIND] failed to inspect device binding: {e}", flush=True)

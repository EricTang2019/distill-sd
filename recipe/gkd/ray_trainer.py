...
class OnPolicyDistillTrainer(RayPPOTrainer):
    ...
    def init_workers(self):
        ...
        # Start async rollout servers (vllm_server_*/sglang_server_*) for ServerAdapter-based rollout.
    ...
    @staticmethod
    def _normalize_key_set(value) -> set[str]:
        if value is None:
            return set()
    ...
    def _resolve_non_tensor_keep_keys(self, non_tensor_keys) -> set[str]:
        non_tensor_keys = set(non_tensor_keys)
        keep_keys = {"uid"}

        # Keep reward-related metadata based on config instead of hard-coded names.
        keep_keys |= self._normalize_key_set(OmegaConf.select(self.config, "data.reward_fn_key", default=None))

        # Optional extension point for custom datasets/reward pipelines.
        keep_keys |= self._normalize_key_set(OmegaConf.select(self.config, "data.keep_non_tensor_keys", default=None))

        # Keep any obviously reward-like fields by key naming convention.
        keep_keys |= {k for k in non_tensor_keys if ("reward" in k) or (k == "extra_info")}

        return keep_keys & non_tensor_keys
    ...
    def _async_gen_next_batch(self, epoch, batch_dict, sync_before_generation=True):
        ...
        # Keep reward-related metadata in `batch`; move generation inputs to `gen_batch`.
        # New RLHFDataset returns `raw_prompt` (non-tensor) + `dummy_tensor` (tensor),
        # while older versions used tokenized tensor keys directly.
        reward_model_keys = self._resolve_non_tensor_keep_keys(batch.non_tensor_batch.keys())
        batch_keys_to_pop = []
        non_tensor_batch_keys_to_pop = [k for k in batch.non_tensor_batch.keys() if k not in reward_model_keys]
        gen_batch = batch.pop(
            batch_keys=batch_keys_to_pop,
            non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
        )
        gen_batch.meta_info["global_steps"] = self.global_steps
        if "raw_prompt" not in gen_batch.non_tensor_batch and "prompt" in gen_batch.non_tensor_batch:
            gen_batch.non_tensor_batch["raw_prompt"] = gen_batch.non_tensor_batch["prompt"]
        # sync weights from actor to rollout
        if sync_before_generation:
            self.sync_rollout_weights()
        # Use async server manager for generation because ServerAdapter no longer supports sync generate().

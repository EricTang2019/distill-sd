...
def get_zmq_namespace() -> str:
    """Return a stable IPC namespace shared by rollout sender/receiver."""
    namespace = os.environ.get("VERL_ZMQ_NAMESPACE")
    if namespace:
        return namespace
    ...
    # Persist in-process so all callers in this worker use the same value.
    os.environ["VERL_ZMQ_NAMESPACE"] = namespace
    return namespace


def get_zmq_handle(device_uuid: str) -> str:
    """Build the IPC socket path for a device UUID."""
    return f"ipc:///tmp/rl-colocate-zmq-{get_zmq_namespace()}-{device_uuid}.sock"


def get_zmq_timeout_ms() -> int:
    """Get REQ/REP timeout for rollout weight sync sockets."""
    timeout_sec_str = os.environ.get("VERL_ZMQ_TIMEOUT_SEC", "300")
    try:
...
class vLLMColocateWorkerExtension:
    ...
    def update_weights_from_ipc(self, peft_config: dict = None, base_sync_done=False, use_shm: bool = False):
        ...
        logger.warning(
            "vLLM update_weights_from_ipc waiting on %s (timeout=%ss, use_shm=%s)",
            zmq_handle,
            timeout_sec,
            use_shm,
        )
        ...
        try:
            ...
            raise TimeoutError(
                f"Timed out waiting for initial IPC metadata on {zmq_handle} after {timeout_sec}s."
            ) from e
        ...
        try:
            ...
            raise TimeoutError(f"Timed out sending initial IPC ack on {zmq_handle} after {timeout_sec}s.") from e
        ...
        while True:
            try:
                ...
                raise TimeoutError(f"Timed out waiting for weight bucket on {zmq_handle} after {timeout_sec}s.") from e
            weights, tensor = [], None
            ...
            try:
                ...
                raise TimeoutError(f"Timed out sending weight bucket ack on {zmq_handle} after {timeout_sec}s.") from e
    ...
    def _get_zmq_handle(self) -> str:
        ...
        return get_zmq_handle(self.device_uuid)
